// SPDX-License-Identifier: Apache-2.0
//
//! **Cerebras** LLM — a thin **(W)rapper** over [`OpenAiLlm`](super::OpenAiLlm).
//!
//! Cerebras exposes an **OpenAI-compatible** chat-completions endpoint (verified
//! against pipecat `services/cerebras/llm.py`, which is
//! `class CerebrasLLMService(OpenAILLMService)`), so this is the reference [`OpenAiLlm`]
//! pointed at [`CEREBRAS_API_BASE`] with the [`CEREBRAS_DEFAULT_MODEL`] default. Behind the
//! `llm-cerebras` feature (which enables `llm-openai`).

use async_trait::async_trait;
use futures::stream::BoxStream;

use flowcat_core::error::Result;
use flowcat_core::processor::frame::{Frame, LlmContext, StartParams};
use flowcat_core::service::{LlmService, Tool};

use super::{OpenAiLlm, OpenAiLlmBuilder};

/// Cerebras's OpenAI-compatible API base.
pub const CEREBRAS_API_BASE: &str = "https://api.cerebras.ai/v1";
/// Cerebras's default model.
pub const CEREBRAS_DEFAULT_MODEL: &str = "gpt-oss-120b";

/// Cerebras LLM service — an [`OpenAiLlm`] pointed at the Cerebras base URL.
pub struct CerebrasLlm {
    inner: OpenAiLlm,
}

impl CerebrasLlm {
    /// Construct bound to `api_key`, using [`CEREBRAS_DEFAULT_MODEL`].
    pub fn new(api_key: impl Into<String>) -> Self {
        Self::with_model(api_key, CEREBRAS_DEFAULT_MODEL)
    }

    /// Construct bound to `api_key` with an explicit `model`.
    pub fn with_model(api_key: impl Into<String>, model: impl Into<String>) -> Self {
        let inner = OpenAiLlmBuilder::new(api_key)
            .base_url(CEREBRAS_API_BASE)
            .model(model)
            .build();
        Self { inner }
    }
}

#[async_trait]
impl LlmService for CerebrasLlm {
    fn name(&self) -> &str {
        "cerebras"
    }

    async fn start(&mut self, params: &StartParams) -> Result<()> {
        self.inner.start(params).await
    }

    async fn run_llm<'a>(&'a mut self, ctx: &'a LlmContext) -> Result<BoxStream<'a, Frame>> {
        self.inner.run_llm(ctx).await
    }

    fn set_tools(&mut self, tools: Vec<Tool>) {
        self.inner.set_tools(tools)
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn constructs_with_base_and_default_model() {
        // Construction must not panic; the name is the provider id.
        assert_eq!(CerebrasLlm::new("k").name(), "cerebras");
        assert_eq!(CerebrasLlm::with_model("k", "custom").name(), "cerebras");
    }
}
